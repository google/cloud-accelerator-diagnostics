...
"""PyGrain data loading metric provider for TPU Info.

This module registers metrics for the PyGrain data loading pipeline, mapping
...
def _make_pygrain_fetcher(
    name: str, path: str
) -> Callable[..., list[console.RenderableType]]:
  """Creates and registers a PyGrain metric retrieval handler with the registry.
  ...
  MetricRegistry under the 'pygrain' group, and returns the fetcher callable.

  Args:
    name: The short metric name (e.g., 'pygrain_read_count').
  ...
  @registry.register_metric(name=name, group="pygrain", description=description)
  def fetcher(
      *,
    ...
    """Retrieves or extracts PyGrain metric tables.
    ...
    Returns:
      A list of Rich renderable tables for the PyGrain metric.
    """
...
for metric_name, prom_path in metrics.PYGRAIN_SHORT_TO_LONG_MAP.items():
  globals()[f"get_{metric_name}"] = _make_pygrain_fetcher(
      metric_name, prom_path
  )
